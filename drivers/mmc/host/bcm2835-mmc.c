 ...
 *
 * Authors:	Piotr Król <piotr.krol@3mdeb.com>
 ...
 *
 * 22/10/2014	Piotr Król checkpatch cleanup
 *
...
struct bcm2835_host {
	...
	/* Waitqueue for Buffer Read Ready IRQ */
...
static inline void bcm2835_mmc_writel(struct bcm2835_host *host, u32 val,
				      int reg)
{
...
static inline void bcm2835_mmc_writew(struct bcm2835_host *host, u16 val,
				      int reg)
{
...
static inline void bcm2835_mmc_writeb(struct bcm2835_host *host, u8 val,
				      int reg)
{
...
static void bcm2835_mmc_prepare_data(struct bcm2835_host *host,
	...
	struct mmc_command *cmd)
{
	...
	if ((mmc_op_multi(cmd->opcode) || data->blocks > 1)) {
		...
		if (!host->mrq->sbc && (host->flags & SDHCI_AUTO_CMD12))
			...
			bcm2835_mmc_writel(host, host->mrq->sbc->arg,
					   SDHCI_ARGUMENT2);
		}
	}
...
void bcm2835_mmc_send_command(struct bcm2835_host *host,
			      struct mmc_command *cmd)
{
	int flags;
	u32 mask;
	...
	bcm2835_mmc_writew(host, SDHCI_MAKE_CMD(cmd->opcode, flags),
			   SDHCI_COMMAND);
}
...
static void bcm2835_mmc_finish_command(struct bcm2835_host *host)
{
	...
	if (host->cmd->flags & MMC_RSP_PRESENT) {
		if (host->cmd->flags & MMC_RSP_136) {
			...
			host->cmd->resp[0] = bcm2835_mmc_readl(host,
							       SDHCI_RESPONSE);
		}
	}
...
static void bcm2835_mmc_enable_sdio_irq_nolock(struct bcm2835_host *host,
					       int enable)
{
...
static void bcm2835_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (host->pwr != SDHCI_POWER_330) {
		...
		bcm2835_mmc_writeb(host, SDHCI_POWER_330 | SDHCI_POWER_ON,
				   SDHCI_POWER_CONTROL);
	}
...
int bcm2835_mmc_add_host(struct bcm2835_host *host)
{
	...
	setup_timer(&host->timer, bcm2835_mmc_timeout_timer,
		    (unsigned long)host);
